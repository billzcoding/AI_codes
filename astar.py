...
def astar(graph, coords, start, goal):
    ...
    pq.put((heuristics[start], start, [start], 0))  # (f = g + h, node, path, g)

    visited = {}  # stores best known g-costs
    ...
    while not pq.empty():
        ...
        for neighbor, cost in graph[current]:
            ...
            pq.put((f, neighbor, path + [neighbor], new_g))
...
path, cost = astar(graph, coordinates, start_node, goal_node)
